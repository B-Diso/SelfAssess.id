...
class GetSystemAssessmentSummary
{
    /**
     * Get system-wide assessment summary by status.
     */
    public function handle(): array
    {
        // Count by status
        $byStatus = Assessment::select('status', DB::raw('count(*) as count'))
            ->groupBy('status')
            ->pluck('count', 'status')
            ->toArray();
        ...
        $recent = Assessment::with(['organization', 'responses'])
            ->latest()
            ->limit(5)
            ->get()
